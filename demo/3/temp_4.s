...
;SCREEN_VOFFSET = 0;(h/2-(FONT_HEIGHT/2))*ScrBpl*bpls
SCREEN_VOFFSET = 10*ScrBpl*bpls
...
; from right word and ends to left word
	...
	move.l	#SCREEN+SCREEN_VOFFSET+(FONT_HEIGHT*ScrBpl*bpls)-2,d0 ; source and dest address

	BLTWAIT BWT2

	move.w	#$49f0,BLTCON0(a5)	; BLTCON0 copy from A to D ($F) 
					; 1 pixel shift, LF = F0
...
; from right word and ends to left word
	...
	lea	SCREEN+SCREEN_VOFFSET,a1 ; source and dest address

	move.w	#$09f0,BLTCON0(a5)	; BLTCON0 copy from A to D ($F) 
					; 1 pixel shift, LF = F0
	move.w	#$0000,BLTCON1(a5)	; BLTCON1 use blitter DESC mode
