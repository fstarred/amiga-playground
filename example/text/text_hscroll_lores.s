...
scroll_left:
	cmpi.b	#$ff,OWNBPLCON1	; check right edge scroll reached
...
set_bpl_right:	
	...
	move.b	#$ff,OWNBPLCON1	; scroll hardware 15 - BPLCON1 ($dff102)
...
print_row:
	moveq	#80-1,d0	; number of columns for line
print_char:
	moveq	#0,d2		; clean d2
	...
	move.b	(a2)+,80(a3)	; print line 2 font on BITPLANE
	move.b	(a2)+,80*2(a3)	; print line 3 font on BITPLANE
	move.b	(a2)+,80*3(a3)	; print line 4 font on BITPLANE
	move.b	(a2)+,80*4(a3)	; print line 5 font on BITPLANE
	move.b	(a2)+,80*5(a3)	; print line 6 font on BITPLANE
	move.b	(a2)+,80*6(a3)	; print line 7 font on BITPLANE
	move.b	(a2)+,80*7(a3)	; print line 8 font on BITPLANE

	addq.w	#1,a3		; move on next char (8bit)

	dbra	d0,print_char	; print 40 chars each line

	add.w	#80*7,a3	; go to next row
				; 
...
TEXT:		
	...
	dc.b	'row this time                           ' ; 1b
	dc.b	'                                        ' ; 2a
	dc.b	'                                        ' ; 2b
	dc.b	'                                        ' ; 3
	dc.b	'                                        ' ; 
	dc.b	'   Looks like we don t care about space ' ; 4
	dc.b	'because this time we have an            ' ; 
	dc.b	'      awesome text                      ' ; 5
	dc.b	'                                        ' ; 
	dc.b	'                                        ' ; 6
	dc.b	'                                        ' ; 
...
OWNBPLCON1:
	...
	dc.w	$104,0		; BplCon2
	dc.w	$108,40-2	; Bpl1Mod (40 for large pic)
	dc.w	$10a,40-2	; Bpl2Mod -2 fits the DdfStart value
